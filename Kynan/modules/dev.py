...
def allow_groups(update: Update, context: CallbackContext):
    args = context.args
    if not args:
        state = "Lockdown is " + "on" if not KynanRobot.ALLOW_CHATS else "off"
        update.effective_message.reply_text(f"Current state: {state}")
        return
    if args[0].lower() in ["off", "no"]:
        KynanRobot.ALLOW_CHATS = True
    elif args[0].lower() in ["yes", "on"]:
        KynanRobot.ALLOW_CHATS = False
    else:
    ...
    if args:
        ...
        try:
            ...
            update.effective_message.reply_text(
                "Beep boop, saya tidak bisa keluar dari grup itu (entah kenapa tho).",
            )
            return
...
def gitpull(update: Update, context: CallbackContext):
    ...
    sent_msg_text = sent_msg.text + "\n\nPerubahan menarik...Saya kira .. Mulai ulang "
